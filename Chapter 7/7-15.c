...
int main(void)
{
   ...
   const char *face[FACES] =
	...
	{
		printf("Do you want to take a new card instead %s of %s(1 = yes, 0 = no)\n", face[cardFace[1][i]], suit[cardSuit[1][i]]);
		scanf("%d", &valid);
		
		if(valid == 1)
		{
			deal(deck, face, suit, addCardF, addCardS, 1);
			
			printf("New card is %s of %s\n", face[addCardF[0]], suit[addCardS[0]]);
			
			cardFace[1][i] = addCardF[0];
			cardSuit[1][i] = addCardS[0];
			valid1 = 1; 
			count++;	
		}
	}
	...
	if(valid1 == 1)
	{
        ...
        cardModelArr(cardFace[1], faceModel[1]);
		pokerHand[1] = 0;
		
		for(int i = 0; i < HANDS - 1; i++)
		{
			pokerHand[1] = (*f[i])(faceModel[1], cardSuit[1]);
